...
if [ "$NEEDS_ARCH_UPDATE" = true ]; then
    echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${YELLOW}⚠️  Architecture Documentation Check${NC}"
    echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    ...
    if echo "$CHANGED_FILES" | grep -q "docs/architecture/diagrams.md"; then
        echo -e "${GREEN}✅ docs/architecture/diagrams.md is included in this commit${NC}"
        echo ""
    else
        echo -e "${RED}❌ docs/architecture/diagrams.md was NOT updated${NC}"
        echo ""
        echo -e "${YELLOW}Consider updating the architecture diagrams:${NC}"
        echo ""
        echo "  1. Review .claude/instructions.md for guidance"
        echo "  2. Update relevant diagrams in docs/architecture/diagrams.md"
        echo "  3. Stage the changes: git add docs/architecture/diagrams.md"
        echo ""
        echo -e "${YELLOW}Quick checklist - did you add/change:${NC}"
        echo "  • New API endpoint?"
        echo "  • New database table?"
        echo "  • New service/component?"
        echo "  • New MCP tool?"
        echo "  • Data flow between components?"
...
# Validate Mermaid syntax if docs/architecture/diagrams.md changed
if echo "$CHANGED_FILES" | grep -q "docs/architecture/diagrams.md"; then
    ...
    if command -v mmdc &> /dev/null; then
        ...
        if mmdc -i docs/architecture/diagrams.md -o /tmp/mindscout_arch_test.png 2>&1 | grep -i "error"; then
            echo -e "${RED}❌ Mermaid syntax errors found!${NC}"
            echo ""
            echo "Please fix syntax errors before committing."
            echo "Test your diagrams at: https://mermaid.live"
            exit 1
        else
            echo -e "${GREEN}✅ Mermaid diagrams are syntactically valid${NC}"
            rm -f /tmp/mindscout_arch_test.png
        fi
    else
        echo -e "${YELLOW}⚠️  mermaid-cli not installed, skipping validation${NC}"
        echo "  Install with: npm install -g @mermaid-js/mermaid-cli"
    fi
fi

echo -e "${GREEN}✅ Architecture documentation check passed${NC}"
exit 0
