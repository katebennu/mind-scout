...
**IMPORTANT**: Whenever you make changes to the following, you MUST update `docs/architecture/diagrams.md`:
...
Open `docs/architecture/diagrams.md` and locate the relevant diagram.
...
### Step 4: Update docs/architecture/overview.md (Text Version)
Update the corresponding text sections in `docs/architecture/overview.md` to match the diagrams.

### Step 5: Commit with Clear Message
```bash
git add docs/architecture/diagrams.md docs/architecture/overview.md
...
# Check if docs/architecture/diagrams.md has valid Mermaid syntax
if git diff --cached --name-only | grep -q "docs/architecture/diagrams.md"; then
    echo "Validating Mermaid diagrams..."

    # Install: npm install -g @mermaid-js/mermaid-cli
    if command -v mmdc &> /dev/null; then
        mmdc -i docs/architecture/diagrams.md -o /tmp/test.png 2>&1 | grep -i error
        if [ $? -eq 0 ]; then
...
1. **Design Phase**
   ...
   # Add to docs/architecture/diagrams.md with [DRAFT] label
   ```
...
code docs/architecture/diagrams.md
